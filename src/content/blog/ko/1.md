...
최근 몇 년간 데이터 검증의 중요성이 커지면서 [JSON Schema](#json-schema)를 중심으로 다양한 검증 라이브러리가 등장했습니다. 대표적으로 [Zod](#zod), [Ajv](#ajv), [class-validator](#class-validator) 등이 있으며, 저 또한 JSON Schema의 가능성에 매료되어 이 분야를 탐구하게 되었습니다.

처음에는 Ajv를 사용하려 했으나, 개발 과정에서 몇 가지 한계점을 발견했고, 이는 결국 제가 직접 라이브러리를 개발하게 된 계기가 되었습니다. 이 글에서는 제가 겪은 기술적 도전과 해결 과정, 그리고 그 과정에서 얻은 인사이트를 공유하고자 합니다.

## 1. 기존 라이브러리의 한계와 도전 계기

[Ajv](#ajv)는 JSON Schema 생태계에서 가장 널리 사용되는 라이브러리지만, TypeScript 환경에서 다음과 같은 타입 추론의 한계가 있었습니다.
...
물론, 이 문제는 [Ajv Utility Types](#ajv-typescript)로 어느 정도 해결할 수 있지만, 이는 **스키마와 타입 정의를 이중으로 관리해야 하는** 새로운 문제를 야기합니다. 이상적으로는, 스키마 정의 자체가 타입 정보를 제공할 수 있어야 한다고 생각했습니다.
...
새로운 라이브러리를 개발하면서 마주친 가장 큰 두 가지 과제는 테스트 자동화와 Format 키워드 구현이었습니다.

### 3-1. 테스팅 자동화

JSON Schema 팀에서는 [JSON-Schema-test-suite](#JSON-Schema-Test-Suite) 저장소를 통해 JSON Schema 형식의 테스트 케이스를 제공하고 있었습니다.
...
1. 저장소를 동기화
2. 디렉토리를 읽어 버전별 테스트 파일 수집
3. [TS Compiler API](#ts-compiler-api)로 타입 정의 생성
4. 생성된 타입을 활용한 테스트 함수 작성

#### 3-1-2. 구현 과정

먼저 저장소 동기화를 위한 `cli` 를 작성했습니다.
...
그리고 테스트 실행을 위한 관리자 클래스를 만들었습니다.
...
하지만 이 접근 방식에는 중요한 문제가 있었습니다.

1. **성능 이슈**: 동적으로 파일을 읽어들이는 방식은 실행 시간이 느렸습니다.
2. **복잡성**: 테스트 설정과 실행 과정이 불필요하게 복잡했습니다.

이러한 문제를 해결하기 위해 TS Compiler API를 활용한 **테스트 파일 자동 생성 CLI 도구**를 작성했습니다.

하지만, 이미 JSON Schema 팀이 [Bowtie](#bowtie)라는 강력한 CLI 테스팅 도구를 제공하고 있었습니다.
...
우선 반복적으로 사용되는 [Formatter](#formatter) 를 쉽게 정의할 수 있도록, 먼저 [Serializable Decorator](#serializable-decorator)를 구현하였습니다.
...
`Decorator` 로 만들게 된 이유는 `Generic`을 통해 `Abstract Implement Class`([관련 이슈](#abstract-implement-class-related-issue)) 처럼 활용도 할 수 있고
특정 메서드를 자동으로 구현할 수 있도록 하기 위해서였습니다.
...
@Serializable
// ^^^^^^^^^^
// `parse`, `stringify`, `safeParse` 메서드가 구현되어 있지 않은 경우
// 오류를 발생시킵니다.
class MyClass { ... }
```
...
class MyClass {
  ...
  // `toString()`, `toJSON()`, `safeParse()` 메서드를 자동으로 구현합니다.
}
```

#### 3-2-3. ABNF 문법과 정규식

RFC 문서에서 제공하는 [ABNF](#abnf-wiki) 문법을 정규식으로 변환하는 과정에서 코드의 재사용성과 디버깅에 어려움이 있었습니다. 특히 [RFC 3986](#rfc-3986)의 `uri` 구현에서 이러한 문제가 두드러졌습니다.

예를 들어, 다음과 같은 정규식은 특정 문자가 누락되거나 업데이트될 경우 문제를 파악하기가 매우 어렵습니다.

```ts
const userinfo = /[a-zA-Z0-9\-._~!$&'()*+,;=:]+/;
```

이러한 문제를 해결하기 위해 [Builder Pattern](#builder-pattern)을 적용한 [@imhonglu/pattern-builder](#imhonglupattern-builder) 라이브러리를 개발했습니다. 이 라이브러리를 통해 ABNF 문법을 더욱 명확하고 유지보수가 용이한 코드로 작성할 수 있게 되었습니다.

먼저 기본이 되는 패턴들을 다음과 같이 정의합니다.
...
이렇게 정의된 기본 패턴들을 조합하여 더 복잡한 패턴을 구성할 수 있습니다.
...
이러한 단계적 패턴 구성 방식은 다음과 같은 장점을 제공합니다.

- ABNF 규칙의 의도와 역할을 명확하게 표현할 수 있습니다.
- 각 패턴을 독립적으로 수정하기 쉽습니다.
- 패턴별로 테스트와 디버깅이 가능합니다.
- 정의된 패턴을 다른 규칙에서 쉽게 재사용할 수 있습니다.

위와 같은 개선을 통해 코드의 가독성과 유지보수성이 크게 향상되었고, [@imhonglu/format](#imhongluformat) 라이브러리의 견고한 기반이 되었습니다.
...
쉘 명령어를 실행하기 위한 유틸리티 함수로, Node.js 환경에서 시스템 명령어를 쉽게 실행할 수 있습니다.
